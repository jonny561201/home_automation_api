...
class TestLightState:
    ...
    def test_add_light_alarm__should_cancel_existing_alarm(self, mock_api):
        ...
        self.STATE.add_light_alarm(self.GROUP_ID, self.TIME, self.DAYS)

        assert len(self.STATE.LIGHT_ALARMS) == 1
        test_event.set.assert_called()

    def test_add_light_alarm__should_create_a_new_alarm(self, mock_api):
        existing_alarm = LightAlarm(self.GROUP_ID, self.TIME, self.DAYS)
    ...
    def test_add_light_alarm__should_create_new_alarm_when_does_not_exist(self, mock_api):
