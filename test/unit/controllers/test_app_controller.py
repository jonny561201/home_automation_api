...
class TestAppControllerAccount:
    ...
    def test_save_user_preferences__should_call_insert_preferences_by_user_with_user_id(self, mock_jwt, mock_db):
        ...
        mock_db.return_value.__enter__.return_value.insert_preferences_by_user.assert_called_with(self.USER_ID, ANY, ANY)
    ...
    def test_save_user_preferences__should_pass_garage_node_id_from_request(self, mock_jwt, mock_db):
        mock_jwt.get_instance.return_value.verify_jwt.return_value = self.CLAIMS
        node_id = str(uuid.uuid4())
        user_preferences = {'city': 'Berlin', 'garageNodeId': node_id}

        save_user_preferences(self.BEARER_TOKEN, user_preferences)

        mock_db.return_value.__enter__.return_value.insert_preferences_by_user.assert_called_with(self.USER_ID, user_preferences, node_id)
