...
class TestSettings:
    ...
    def test_db_pass__should_pull_from_settings(self):
        assert self.SETTINGS.Database.password == self.db_settings['Password']

    def test_db_port__should_pull_from_settings(self):
        assert self.SETTINGS.Database.port == self.db_settings['Port']

    def test_db_name__should_pull_from_settings(self):
        assert self.SETTINGS.Database.name == self.db_settings['Name']

    def test_email_app_id__should_pull_from_settings(self):
        assert self.SETTINGS.email_app_id == self.test_settings['EmailAppId']

    def test_weather_app_id__should_pull_from_settings(self):
    ...
    def test_light_api_key__should_pull_from_settings(self):
        assert self.SETTINGS.light_api_key == self.test_settings['LightApiKey']

    def test_queue_user_name__should_pull_from_settings(self):
        assert self.SETTINGS.Queue.user_name == self.q_settings['User']

    def test_queue_password__should_pull_from_settings(self):
        assert self.SETTINGS.Queue.password == self.q_settings['Password']

    def test_queue_host__should_pull_from_settings(self):
        assert self.SETTINGS.Queue.host == self.q_settings['Host']

    def test_queue_vhost__should_pull_from_settings(self):
