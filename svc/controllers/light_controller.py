...
def get_assigned_light_groups(bearer_token):
    ...
    return api_utils.get_light_groups()
...
def set_assigned_light_groups(bearer_token, request):
    ...
    # api_key = light_state.get_light_api_key()
    try:
        api_utils.set_light_groups(request['groupId'], request['on'], request.get('brightness'))
    except KeyError:
        raise BadRequest()
...
def set_assigned_light(bearer_token, request_data):
    ...
    api_utils.set_light_state(request_data.get('lightId'), request_data.get('brightness'))
